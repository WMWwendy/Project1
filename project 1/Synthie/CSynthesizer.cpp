   ...
   //
    ...
    for (list<CInstrument*>::iterator node = m_instruments.begin(); node != m_instruments.end();)
    {
        ...
        if (instrument->Generate())
        {
            ...
            if (m_noisegating == true)
            {
                double eframe[2];
                m_nise.Process(frame, eframe);
                for (int c = 0; c < GetNumChannels(); c++)
                {
                    frame[c] += instrument->Frame(c);
            ...
            else if (m_chorus == true)
            {
                double eframe[2];
                m_ch.Process(frame, eframe);
            ...
            else if (m_flanging == true)
            {
                double eframe[2];
                m_fl.Process(frame, eframe);
